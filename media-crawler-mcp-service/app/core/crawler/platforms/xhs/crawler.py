...
class XiaoHongShuCrawler:
    ...
    async def close(self):
        ...
        try:
            if self.browser_context:
                # 使用浏览器管理器释放上下文，而不是直接关闭
                await browser_manager.release_context(Platform.XIAOHONGSHU.value)
            logger.info("[XiaoHongShuCrawler.close] Browser context released")
        except Exception as e:
    ...
    async def _ensure_browser_and_client(self) -> None:
        """确保浏览器上下文和客户端已准备就绪"""
        # 检查现有浏览器上下文是否有效
        if self.browser_context and self.context_page:
            try:
                # 尝试检查页面是否可用
                await self.context_page.evaluate("1 + 1")
            except (TargetClosedError, RuntimeError):
                # 页面或上下文已关闭，需要重新获取
                logger.warning("[xhs.browser] 检测到页面已关闭，重新获取浏览器上下文")
                self.browser_context = None
        ...
        if not self.browser_context or not self.client:
            ...
            if self.browser_context:
                try:
                    ...
                    logger.warning(f"[xhs.browser] 释放上下文失败: {e}")
            ...
            for retry in range(max_retries):
                try:
                    await self.context_page.goto("https://www.xiaohongshu.com", timeout=30000)
                    break
                except TargetClosedError:
                    logger.error(f"[xhs.browser] 重试 {retry+1}/{max_retries}: 页面已关闭")
                    # 重新获取浏览器上下文
                    await browser_manager.release_context(Platform.XIAOHONGSHU.value)
                    self.browser_context, self.context_page, playwright = await browser_manager.acquire_context(
                        platform=Platform.XIAOHONGSHU.value,
                    ...
                    logger.error(f"[xhs.browser] 重试 {retry+1}/{max_retries}: 导航失败: {e}")
                    if retry == max_retries - 1:
                        raise
                    await asyncio.sleep(1)
